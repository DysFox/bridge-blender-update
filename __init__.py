...
# Set socket index variables
## Diffuse
globals()['DiffuseSocketIndex'] = 0

## Metallic
if bpy.app.version >= (3,0,0):
    globals()['MetallicSocketIndex'] = 6
else:
    globals()['MetallicSocketIndex'] = 4

## Specular
if bpy.app.version >= (3,0,0):
    globals()['SpecularSocketIndex'] = 7
else:
    globals()['SpecularSocketIndex'] = 5

## Roughness
if bpy.app.version >= (3,0,0):
    globals()['RoughnessSocketIndex'] = 9
else:
    globals()['RoughnessSocketIndex'] = 7

## IOR
if bpy.app.version >= (3,0,0):
    globals()['IORSocketIndex'] = 16
else:
    globals()['IORSocketIndex'] = 14

## Transmission
if bpy.app.version >= (3,0,0):
    globals()['TransmissionSocketIndex'] = 17
else:
    globals()['TransmissionSocketIndex'] = 15

## Emission
if bpy.app.version >= (3,0,0):
    globals()['EmissionSocketIndex'] = 19
else:
    globals()['EmissionSocketIndex'] = 17

## Alpha
if bpy.app.version >= (3,0,0):
    globals()['AlphaSocketIndex'] = 21
elif bpy.app.version >= (2,91,0):
    globals()['AlphaSocketIndex'] = 19
else:
    globals()['AlphaSocketIndex'] = 18

## Normal
if bpy.app.version >= (3,0,0):
    globals()['NormalSocketIndex'] = 22
elif bpy.app.version >= (2,91,0):
    globals()['NormalSocketIndex'] = 20
else:
    globals()['NormalSocketIndex'] = 19
    ...
    def SetupMaterial (self):
        ...
        if self.isSpecularWorkflow:
            if "specular" in self.textureTypes:
                self.CreateTextureNode("specular", -1150, 200, 0, True, globals()['SpecularSocketIndex'])
            ...
            if "gloss" in self.textureTypes:
                ...
                self.mat.node_tree.links.new(self.nodes.get(self.parentName).inputs[globals()['RoughnessSocketIndex']], invertNode.outputs[0])
            elif "roughness" in self.textureTypes:
                self.CreateTextureNode("roughness", -1150, -60, 1, True, globals()['RoughnessSocketIndex'])
        else:
            if "metalness" in self.textureTypes:
                self.CreateTextureNode("metalness", -1150, 200, 1, True, globals()['MetallicSocketIndex'])
        ...
        if "opacity" in self.textureTypes:
            self.CreateTextureNode("opacity", -1550, -160, 1, True, globals()['AlphaSocketIndex'])
            self.mat.blend_method = 'HASHED'

        if "translucency" in self.textureTypes:
            self.CreateTextureNode("translucency", -1550, -420, 0, True, globals()['TransmissionSocketIndex'])
        elif "transmission" in self.textureTypes:
            self.CreateTextureNode("transmission", -1550, -420, 1, True, globals()['TransmissionSocketIndex'])

        # If HIGH POLY selected > use normal_bump and no displacement
        # If LODs selected > use corresponding LODs normal + displacement
        if self.isHighPoly:
            self.BumpSetup = False
        self.CreateNormalNodeSetup(True, globals()['NormalSocketIndex'])
